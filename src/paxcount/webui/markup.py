...
def _sources() -> list[Path]:
    """Боевые записи всех камер. Отладочный набор не входит: эталон строится по бою.

    К1 сначала была убрана: разметка дверей нужна для счёта, а по ней не
    считают (решение 030). Но то же решение называет её незаменимой для
    другого — с её кадра читается бортовой номер и виден порядок в очереди, —
    и, убрав запись с экрана, мы закрыли заодно и это. Она вернулась с
    пометкой: смотреть можно, размечать двери незачем.
    """
    if not PROD_VIDEO_DIR.is_dir():
        return []
    return list(videos_in(PROD_VIDEO_DIR))


def listing_row(meta: dict) -> dict:
    """Строка списка файлов: метаданные записи и ведётся ли по ней счёт."""
    return {**meta, "counts": cameras.counts(meta.get("camera", ""))}
...
def api_files() -> JSONResponse:
    ...
    rows = sorted(
        (listing_row(_meta(p)) for p in _sources()),
        key=lambda r: (r["camera"], r["start"] or ""),
    )
    return JSONResponse({
        "files": rows,
        "layouts": _saved_layouts(),
        # Камеры показываются все, а несчётные помечены в самой строке. Список
        # исключённых остался пустым списком, чтобы старый экран не падал.
        "excluded": [],
    })
