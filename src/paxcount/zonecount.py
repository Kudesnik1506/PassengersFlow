...
def _leaves_vehicle(life: TrackLife) -> tuple[bool, str]:
    """Уходит ли человек ПОПЕРЁК корпуса сразу после появления.

    Вышедший из ТС делает шаг наружу — поперёк борта. Прохожий, у которого трек
    порвался из-за перекрытия, идёт ВДОЛЬ корпуса, почти горизонтально. Вот это
    и есть разделяющий признак, и он сохраняется.

    Сторона при этом не важна: на К2 и К3 дверь смотрит вдоль кадра, и вышедший
    уходит от камеры так же часто, как к ней. Требование «вниз по кадру»
    отсекало вторую половину — замер после починки зоны (060) и порога (063):
    выходы 3 → 4 из 6, входы те же 18, ложных ни на одном визите, отладочный
    набор не сдвинулся. Разрешено владельцем, решение 065.

    Ослабление ДО КОНЦА — «удаление от проёма в любую сторону», то есть без
    поперечности — остаётся отменённым (решение 055): без неё вышедший вдоль
    борта и прохожий вдоль борта совпадают, и замер дважды показал размен один
    к одному.
    """
...
def _shift_is_outward(points, box, sign: int, signed: bool = True) -> tuple[bool, str]:
    """Сдвинулся ли человек поперёк корпуса; `sign` — в какую сторону.

    ``signed=False`` снимает проверку стороны, оставляя поперечность. Нужно
    выходу: на К2 и К3 дверь смотрит вдоль кадра, и вышедший уходит от камеры
    так же часто, как к ней. Требование «вниз по кадру» отсекало вторую
    половину. Ослабление до «в любую сторону» — вместе с поперечностью — здесь
    отменено (решение 055): без поперечности вышедший сливается с прохожим.
    """
