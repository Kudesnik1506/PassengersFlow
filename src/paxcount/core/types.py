...
class DoorSpec(BaseModel):
    ...
    band: float = 0.25
    # Зона двери в долях bbox ТС: (x0, y0, x1, y1). По умолчанию — нижние две
    # трети корпуса: там дверной проём при любом наружном ракурсе.
    zone: tuple[float, float, float, float] = (0.0, 0.33, 1.0, 1.10)
