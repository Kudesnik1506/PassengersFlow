...
class VehicleInfo:
    """Что портал знает о машине на конкретную дату."""
...
class PortalClient:
    ...
    def __init__(
        ...
        # маршрутам, и ответ за вторник не годится для среды.
    ...
    def lookup(self, board_number: str, date: str, kind: VehicleKind) -> Answer:
        ...
        if not rows:
            result: Answer = NotFound(
                board_number,
                f"ТС {board_number} нет в базе портала на {date}",
            )
        else:
            result = _parse(rows[0], kind)
        self._cache[key] = result
        return result


def _parse(row: list, kind: VehicleKind) -> VehicleInfo:
